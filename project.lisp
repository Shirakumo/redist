...
(defmethod checkout ((project project) path &rest args &key (version (version project)) &allow-other-keys)
  ...
  (simple-inferiors:with-chdir (path)
    (loop for source in (sources project)
          do (restart-case (return (cond ((filesystem-utils:empty-directory-p path)
                                          (apply #'clone source :shallow T :version version args))
                                         ((string/= version (version source))
                                          (apply #'update source :version version args))))
               (continue ()
